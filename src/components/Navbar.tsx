...
export default function Navbar({ activeSection, onNavClick, sentinelRef }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex flex-col items-center pt-4 md:pt-6 px-4 pointer-events-none">
      ...
      <div
        className={`pointer-events-auto flex items-center justify-between md:justify-start gap-1 md:gap-1.5 rounded-full border border-white/10 bg-surface/40 px-3 py-2.5 navbar-capsule w-full max-w-[85vw] md:w-auto relative z-50 md:max-w-[95vw] ${isScrolling ? 'backdrop-blur-[3px]' : 'backdrop-blur-md'
      ...
      >
        ...
        >
          ...
          <LiquidGlass.Tab
            ...
            className={`relative text-xs sm:text-sm rounded-full pl-1.5 pr-3 py-1.5 transition-colors duration-200 select-none z-10 flex items-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60 focus-visible:ring-offset-2 focus-visible:ring-offset-black ${active === 'Home'
              ? 'text-text-primary'
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {NAV_LINKS.map(link => (
              <LiquidGlass.Tab
                key={link}
                value={link}
                className={`relative text-xs md:text-sm rounded-full px-3 md:px-4 py-1.5 md:py-2 transition-colors duration-200 select-none z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60 focus-visible:ring-offset-2 focus-visible:ring-offset-black ${active === link
                  ? 'text-text-primary'
                  : 'text-muted hover:text-text-primary'
                  }`}
              >
        ...
        <>
          ...
          <div className="md:hidden z-50 w-72 mt-2 pointer-events-none">
            <LiquidGlass.Tabs
              ...
              style={{
                boxShadow: 'inset 0 1px 1px rgba(255, 255, 255, 0.1), 0 20px 40px -15px rgba(0, 0, 0, 0.7)',
                opacity: isOpen ? 1 : 0,
            ...
            >
              {['Home', ...NAV_LINKS].map(link => (
                <LiquidGlass.Tab
                  key={link}
                  value={link}
                  className={`relative w-full text-center flex justify-center items-center text-sm font-medium rounded-full px-4 py-2.5 transition-colors duration-300 select-none z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60 focus-visible:ring-offset-2 focus-visible:ring-offset-black ${active === link
                    ? 'text-text-primary'

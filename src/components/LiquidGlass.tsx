...
import { motion, useMotionValue, useSpring, useTransform, animate, MotionValue } from 'motion/react';
...
export type LiquidGlassButtonProps = Omit<LiquidGlassProps, 'as' | 'springScale'>;
...
const LiquidGlassComponent = React.forwardRef<HTMLElement, LiquidGlassProps>(({
  ...
  const springPullX = useSpring(pullX, { damping: 30, stiffness: 200, mass: 0.5 });
  const springPullY = useSpring(pullY, { damping: 30, stiffness: 200, mass: 0.5 });
  ...
  }, [interactive, updateRect, mouseX, mouseY, magnetic, pullX, pullY, magneticStrength, tilt, tiltX, tiltStrength, tiltY]);
  ...
  const baseClasses = `
    ...
    overflow-hidden ${(as === 'button' || href || onClick)
  ...
  }, [style, magnetic, springPullX, springPullY, tilt, springTiltX, springTiltY, variant, active, isHovered, specularGlow]);
  ...
  const handlePointerDownWrapper = useCallback((e: React.PointerEvent<HTMLElement>) => {
    if (localRef.current) {
      setWidth(localRef.current.offsetWidth);
    }
    handlePointerDown(e);
  ...
  const sharedAnimationProps = useMemo(() => ({
    whileHover: springScale ? { scaleX: hoverScaleX, scaleY: hoverScaleY } : undefined,
    whileTap: springScale ? { scaleX: tapScaleX, scaleY: tapScaleY } : undefined,
  ...
  }), [springScale, handleMouseEnter, handleMouseMove, handleMouseLeave, handlePointerDownWrapper, hoverScaleX, hoverScaleY, tapScaleX, tapScaleY]);

  // Determine container tag and layout styles dynamically based on semantics
  const ContentTag = (as === 'a' || as === 'button' || as === 'span') ? 'span' : 'div';
  const contentClasses = `relative z-30 w-full h-full ${(as === 'a' || as === 'button' || as === 'span')
  ...
  const innerElements = (
    <>
      {interactive ? (
        <>
          {!isMobile ? (
            <span className={`absolute inset-0 pointer-events-none z-0 overflow-hidden ${roundedClass}`}>
              <motion.span
                ...
                style={{
                  x: springX, y: springY, opacity: springOpacity,
                  left: '50%', top: '50%',
                }}
              />
              <motion.span
                ...
                style={{
                  x: lagX, y: lagY, opacity: springOpacity,
                  left: '50%', top: '50%',
                }}
              />
            </span>
          ...
          {springScale && ripple ? (
            <Ripple clickPos={clickPos} rippleRadius={rippleRadius} rippleOpacity={rippleOpacity} />
          ) : null}
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const Tag = href ? motion.a : as === 'button' ? motion.button : (motion as any)[as];
  ...
  return <Tag ref={localRef} {...tagProps}>{innerElements}</Tag>;
});
...
const LiquidGlassButtonComponent = React.forwardRef<HTMLElement, LiquidGlassButtonProps>(({
  children,
  onClick,
  className = '',
  href,
...
function useTabsContext() {
  ...
  if (!context) {
    throw new Error('LiquidGlass.Tab must be used within a LiquidGlass.Tabs component');
  }
  return context;
}
...
export const Tab = React.memo(function Tab({
  ...
  const { clickPos, rippleRadius, rippleOpacity, onPointerDown } = useRipple(ripple);
  ...
  const stretchDeltaX = isMobileNav ? STRETCH_DELTA_MOBILE : STRETCH_DELTA_DESKTOP;
  const tapDeltaX = isMobileNav ? TAP_DELTA_MOBILE : TAP_DELTA_DESKTOP;

  const HOVER_SCALE_X = 1 + (hoverDeltaX / buttonWidth);
  const HOVER_SCALE_Y = isMobileNav ? HOVER_SCALE_Y_MOBILE : HOVER_SCALE_Y_DESKTOP;

  const TRANSITION_STRETCH_X = 1 + (stretchDeltaX / buttonWidth); // grow wider during transition
  const TRANSITION_SWELL_Y = isMobileNav ? SWELL_SCALE_Y_MOBILE : SWELL_SCALE_Y_DESKTOP;   // grow taller during transition
  ...
  const isNavbarActive = contextHighlightClass?.includes('navbar-highlight-active');

  useEffect(() => {
    const targetX = isPressed ? TAP_SCALE_X : (isNavbarActive ? HOVER_SCALE_X : FLAT_SCALE_X);
    const targetY = isPressed ? TAP_SCALE_Y : (isNavbarActive ? HOVER_SCALE_Y : FLAT_SCALE_Y);

    if (isCurrentlyHighlighted && highlightedTab !== prevHighlightedTabRef.current) {
      const isMobileNav = layoutId?.includes('mobile');
      if (isMobileNav) {
        ...
        animate(scaleY, [scaleY.get(), targetY, TRANSITION_STRETCH_X, targetY], {
          duration: 0.3,
          times: [0, 0.1, 0.4, 1],
          ease: [0.19, 1, 0.22, 1]
        });
        ...
        animate(scaleX, [scaleX.get(), targetX, TRANSITION_STRETCH_X, targetX], {
          duration: 0.3,
          times: [0, 0.1, 0.4, 1],
          ease: [0.19, 1, 0.22, 1]
        });
  ...
  const combinedHighlightClass = `absolute inset-0 z-[-1] highlight-pill overflow-hidden ${roundedClass} ${contextHighlightClass} ${highlightClassName}`.trim();
  ...
  return (
    <motion.button
      ...
      className={`relative select-none z-10 transition-colors duration-200 focus-visible:outline-none ${className} ${isActive ? activeClassName : ''
        }`}
      {...rest}
    >
      {showHighlight ? (
